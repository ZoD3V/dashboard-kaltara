...
import { downloadExcel } from '@/app/(home)/(routes)/(neraca)/helper/download-data-to-excel';
...
import { PriceChangeType, PriceType, PriceTypeRegionValue } from '@/types/price';
...
type NeracaFilterProps = {
    ...
    setSelectedPriceType: (val: PriceType) => void;
...
const NeracaFilter: React.FC<NeracaFilterProps> = ({
    ...
    const [displayedValues, setDisplayedValues] = useState<PriceTypeRegionValue[]>([]);
            ...
            <div className='mb-6 flex flex-col gap-3 px-4 lg:flex-row lg:items-center lg:justify-between'>
                <div className='flex flex-col items-start gap-3 sm:flex-row sm:items-center md:gap-4'>
                    ...
                    <div className='grid w-full grid-cols-1 items-center gap-2 sm:w-fit'>
                        {activeTab === 'price' && (
                            <Select value={selectedPriceType} onValueChange={(v: PriceType) => setSelectedPriceType(v)}>
                ...
                <div className='flex flex-wrap items-center gap-3'>
                    <div className='flex w-full items-center gap-2 sm:w-fit'>
                        <Select
                            onValueChange={(year) => {
                                // penggunaan: gabungkan year + month jadi activeDate
                                const [_, month] = activeDate.split('-');
                        ...
                        <Select
                            onValueChange={(monthLabel) => {
                                // jika mau update activeDate
                                setActiveDate(monthLabel);
                            }}>
                            <SelectTrigger className='w-full sm:w-[200px]'>
